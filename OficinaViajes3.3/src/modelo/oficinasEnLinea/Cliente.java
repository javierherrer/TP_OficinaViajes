...
/**
 *  Cliente comunicación con servidor de oficinas en línea
 * 
 */
public class Cliente {
  ...
  private static String DEBUG_ENVIANDO_SOLICITUD = "Enviando solicitud:";
  private static String DEBUG_DATOS_RECIBIDOS = "Datos recibidos:";
  ...
  private synchronized void enviar(
    ...
    if (Oficina.esModoDebug()) {
      DebugVista.devolverInstancia().mostrar(DEBUG_ENVIANDO_SOLICITUD + " " +
  ...
  private void recibirSolicitudServidor(List<String> resultados) 
    ...
    while (! resultado.equals(PrimitivaComunicacion.FIN.toString())) {        
   ...
   * 
   */  
  private synchronized PrimitivaComunicacion recibir(List<String> resultados, 
                                                     boolean solicitudServidor) 
      throws Exception {   
    ...
    if (Oficina.esModoDebug()) {
      DebugVista.devolverInstancia().mostrar(DEBUG_DATOS_RECIBIDOS + " " +
                                             respuesta.toString());      
    }
    ...
    // En el resto de casos recibimos, si los hay, resultados en líneas sig.    
    resultados.clear();
    ...
    if (Oficina.esModoDebug()) {
      DebugVista.devolverInstancia().mostrar(resultados.toString());      
    }     
  ...
  public synchronized PrimitivaComunicacion enviarSolicitud(
          PrimitivaComunicacion solicitud, 
          int tiempoEspera, 
          String parametros, List<String> resultados) throws Exception {
  ...
  public synchronized void enviarSolicitudLongPolling(
    ...
    do { 
      ...
      if (respuesta != PrimitivaComunicacion.TEST) { 
        if (oyenteServidor.solicitudServidorProducida(respuesta, resultados)) {
          salida.println(PrimitivaComunicacion.OK);
        } else {
